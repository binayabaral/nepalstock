...
const SellCalculator = () => {
	...
	const getGainTax = () => {
		const netBuyingPrice = totalCostPrice + (calculateBrokerCommissionPercentage(totalCostPrice) * totalCostPrice) / 100 + (0.015 * totalCostPrice) / 100 + 25;
		const netSellingPrice = totalSellingPrice - (calculateBrokerCommissionPercentage(totalSellingPrice) * totalSellingPrice) / 100 - (0.015 * totalSellingPrice) / 100 - 25;
	...
	const getProfit = () => {
		const netBuyingPrice = totalCostPrice + (calculateBrokerCommissionPercentage(totalCostPrice) * totalCostPrice) / 100 + (0.015 * totalCostPrice) / 100 + 25;
		const netSellingPrice = totalSellingPrice - (calculateBrokerCommissionPercentage(totalSellingPrice) * totalSellingPrice) / 100 - (0.015 * totalSellingPrice) / 100 - 25 - getGainTax();
		if (netBuyingPrice > 0 && netSellingPrice > 0) {
			return [netSellingPrice - netBuyingPrice, (((netSellingPrice - netBuyingPrice) / netBuyingPrice) * 100).toFixed(2)];
		} else {
			return [0, 0];
		}
	};
		...
		<div className="form-wrapper">
			...
			<form>
				...
				<div className="form-group">
					...
					<span className="result-field">{((calculateBrokerCommissionPercentage(totalSellingPrice) * totalSellingPrice) / 100).toFixed(2)}</span>
				</div>
				<div className="form-group">
				...
				<div className="form-group">
					...
					<span className="result-field jade">{totalCostPrice > 0 ? (totalCostPrice + (calculateBrokerCommissionPercentage(totalCostPrice) * totalCostPrice) / 100 + (0.015 * totalCostPrice) / 100 + 25).toFixed(2) : 0}</span>
				</div>
				<div className="form-group">
					<span className="label">Selling For (Rs.): </span>
					<span className="result-field jade">{totalSellingPrice > 0 ? (totalSellingPrice - (calculateBrokerCommissionPercentage(totalSellingPrice) * totalSellingPrice) / 100 - (0.015 * totalSellingPrice) / 100 - 25 - getGainTax()).toFixed(2) : 0}</span>
				</div>
				<div className="form-group">
